...
function DistributionChart({ data }: { data: { name: string; value: number }[] }) {
  ...
  return (
    <Stack spacing={1.5}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1 }}>{data.map((item, index) => <Stack key={item.name} direction="row" spacing={1} alignItems="center"><Box sx={{ width: 9, height: 9, borderRadius: '50%', bgcolor: pieColors[index % pieColors.length], flexShrink: 0 }} /><Box sx={{ minWidth: 0 }}><Typography variant="caption" color="text.secondary" noWrap>{item.name}</Typography><Typography variant="subtitle2">{item.value.toLocaleString('vi-VN')} <Typography component="span" variant="caption" color="text.secondary">({((item.value / total) * 100).toFixed(1)}%)</Typography></Typography></Box></Stack>)}</Box>
    </Stack>
  );
}
  ...
  return (
    <PageShell title={config.title} subtitle={config.subtitle}>
      ...
      <Box sx={{ display: 'grid', gap: 3, gridTemplateColumns: { xs: '1fr', lg: '2fr 1fr' } }}>
        <SectionCard title={config.trendTitle} subtitle={config.trendSubtitle}><TrendChart data={config.trendData} series={config.trendSeries} /></SectionCard>
        <SectionCard title={config.distributionTitle} subtitle={config.distributionSubtitle}><DistributionChart data={config.distributionData} /></SectionCard>
      </Box>

      <Box sx={{ display: 'grid', gap: 3, gridTemplateColumns: { xs: '1fr', lg: '2fr 1fr' } }}>
        <SectionCard title={config.breakdownTitle} subtitle={config.breakdownSubtitle}><BreakdownChart data={config.breakdownData} series={config.breakdownSeries} /></SectionCard>
        <SectionCard title="Nhận định nổi bật" subtitle="Gợi ý nhanh từ dữ liệu mô phỏng trong kỳ."><Stack spacing={2}>{config.insights.map((insight, index) => <Stack key={insight} direction="row" spacing={1.5} alignItems="flex-start"><Box sx={{ width: 30, height: 30, borderRadius: 1.5, display: 'grid', placeItems: 'center', flexShrink: 0, bgcolor: index === 0 ? 'primary.lighter' : 'background.neutral', color: index === 0 ? 'primary.main' : 'text.secondary' }}><Iconify icon={index === 0 ? 'solar:lightbulb-bold' : 'solar:check-circle-bold'} width={18} /></Box><Typography variant="body2" sx={{ pt: 0.5 }}>{insight}</Typography></Stack>)}{section === 'export' && <Stack spacing={1.25} sx={{ pt: 1 }}><Button variant="contained" startIcon={<Iconify icon="solar:download-bold" />}>Xuất Excel</Button><Button variant="outlined" startIcon={<Iconify icon="solar:file-text-bold" />}>Xuất PDF</Button><Button variant="outlined" startIcon={<Iconify icon="solar:code-bold" />}>Tạo dữ liệu JSON</Button></Stack>}</Stack></SectionCard>
      </Box>
