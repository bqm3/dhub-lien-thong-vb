...
export default function UsersPage() {
  ...
  const { data, isFetching, refetch } = useQuery<{ rows: UserRecord[]; total: number }>({
    ...
    queryFn: async () => {
      ...
      const mapped: UserRecord[] = rawList.map((item: any) => ({
        ...
        unit: item.UNIT_NAME || item.unitName || item.UNIT_CODE || item.unitCode || item.ORG || item.org || 'Chưa phân đơn vị',
        unitCode: item.UNIT_CODE || item.unitCode || item.ORG || item.org || '',
  ...
  function handleSubmit() {
    ...
    saveMutation.mutate({
      ...
      org: formValues.unitCode || formValues.unit,
